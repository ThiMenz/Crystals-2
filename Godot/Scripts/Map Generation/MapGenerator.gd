...
## TO-DO
## -> Base Property Gen: So ziemlich fertig, aber safe line lengths wär ganz nice
## -> Load Biom Graphically
## -> Next Bioms in Load Biom
## -> (Eff.) Save / Load System
...
func LoadBiom():
	var timestamp = Time.get_ticks_usec()
	TestTerrain()
	print("LoadBiom finished in %ms".format([(Time.get_ticks_usec()-timestamp)/1000.], "%"))
...
#region | Embellish Biom |

@export var TestTerrainTexture:Terrain3DTextureAsset
func TestTerrain():
	# Create a terrain
	
	#var cs_class = preload("res://addons/terrabrush/TerraBrushTool.cs")
	#cs_class.EditTerrain(Vector3(0,0,0))
	...
	#terrain.storage.set_height(Vector3(0,0,0), 20.)
	
	var v:Terrain3DTextureList = Terrain3DTextureList.new()
	var tex1 = Terrain3DTexture.new()
	var tex2 = Terrain3DTexture.new()
	tex1.texture_id = 0
	tex2.texture_id = 1
	v.set_textures([tex1, tex2])
	terrain.texture_list = v
	...
	#+	for y in range(0, 1024, 1024):
	#+		print(str(x) + "|" + str(y))
	#+		var thread = Thread.new()
	#+		threads.append(thread)
	#+		thread.start(ThreadFillImageWith128x128Info.bind(img, noise, x, y))
	#+
#+
	#+for t in threads:
	#+	t.wait_to_finish()
	#+		
	#+#var img: Image = $CSharp.GetTerrainImage_CS()
	#+		
	terrain.storage.import_images(
		[img, null, null], # Array of images (heightmap, controlmap, colormap)
		Vector3(0, 0, 0), # Global Position
		0.0, # Offset
		300.0 # Scale (relief)
	)
	
	#terrain.storage.set_height(Vector3(0,0,0), .2)
...
func ThreadFillImageWith128x128Info(img:Image, noise:FastNoiseLite, offsetX:int, offsetY:int):
	for x in 512:
		for y in 1024:
			var tX = offsetX + x
			var tY = offsetY + y
